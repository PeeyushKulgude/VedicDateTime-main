...
#' # ---------------------------------------------------------------------------- #
# ---------------------------------------------------------------------------- #
#' Yoga for a Given Place and Time (Named Return)
#'
#' @description Returns the yoga and its ending time (and leap yoga, if present) for a given Julian day and place.
#'
#' @param jd Julian day number
#' @param place Vector containing latitude, longitude and timezone
#'
#' @return Named list: `today_yoga`, `end_time` (DMS vector), `leap_yoga`, `leap_end_time`
#'
#' @examples
#' yoga(2459778, c(15.34, 75.13, 5.5))
#'
...
yoga <- function(jd, place) {
  swephR::swe_set_sid_mode(swephR::SE$SIDM_LAHIRI, 0, 0)
  tz <- place[3]
  rise <- sunrise(jd, place)$sunrise_jd  # <-- FIXED HERE

  # Nirayana longitudes
  ayan <- swephR::swe_get_ayanamsa_ex_ut(rise, swephR::SE$FLG_SWIEPH + swephR::SE$FLG_NONUT)$daya
  lunar_long <- (moon_longitude(rise) - ayan) %% 360
  solar_long <- (sun_longitude(rise) - ayan) %% 360
  total <- (lunar_long + solar_long) %% 360

  # 27 yogas
  yog <- as.integer(ceiling(total * 27 / 360))

  # Degrees left for this yoga
  degrees_left <- yog * (360 / 27) - total

  # Motion for interpolation
  offsets <- c(0.25, 0.5, 0.75, 1.0)
  lunar_longitude_diff <- sapply(offsets, function(x) (moon_longitude(rise + x) - moon_longitude(rise)) %% 360)
  solar_longitude_diff <- sapply(offsets, function(x) (sun_longitude(rise + x) - sun_longitude(rise)) %% 360)
  total_motion <- lunar_longitude_diff + solar_longitude_diff

  # Today's yoga end time
  approx_end <- inverse_lagrange(offsets, total_motion, degrees_left)
  ends <- (rise + approx_end - jd) * 24 + tz
  end_time <- to_dms(ends)

  # Leap yoga
  ayan_tmrw <- swephR::swe_get_ayanamsa_ex_ut(rise + 1, swephR::SE$FLG_SWIEPH + swephR::SE$FLG_NONUT)$daya
  lunar_long_tmrw <- (moon_longitude(rise + 1) - ayan_tmrw) %% 360
  solar_long_tmrw <- (sun_longitude(rise + 1) - ayan_tmrw) %% 360
  total_tmrw <- (lunar_long_tmrw + solar_long_tmrw) %% 360
  tomorrow <- as.integer(ceiling(total_tmrw * 27 / 360))
  leap_yoga <- NA_integer_
  leap_end_time <- rep(NA_integer_, 3)
  if (((tomorrow - yog) %% 27) > 1) {
    leap_yoga <- yog + 1
    degrees_left_leap <- leap_yoga * (360 / 27) - total
    approx_end_leap <- inverse_lagrange(offsets, total_motion, degrees_left_leap)
    leap_ends <- (rise + approx_end_leap - jd) * 24 + tz
    if (leap_yoga >= 28) {
      leap_yoga <- (leap_yoga %% 28) + 1
    }
