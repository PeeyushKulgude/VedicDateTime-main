...
# ---------------------------------------------------------------------------- #
#' Tithi for a Given Place and Time (Named Return)
#'
#' @description Returns the tithi and its ending time (and leap tithi, if present) for a given Julian day and place.
#'
#' @param jd Julian day number
#' @param place Vector containing latitude, longitude and timezone
#'
#' @return Named list: `today_tithi`, `end_time` (DMS vector), `leap_tithi`, `leap_end_time`
#'
#' @examples
#' tithi(2459778, c(15.34, 75.13, 5.5))
#'
...
tithi <- function(jd, place) {
  ...
  moon_phase <- lunar_phase(rise)
  today <- as.integer(ceiling(moon_phase / 12))
  degrees_left <- today * 12 - moon_phase
  ...
  relative_motion <- lunar_longitude_diff - solar_longitude_diff

  # End time for today's tithi
  approx_end <- inverse_lagrange(offsets, relative_motion, degrees_left)
  ends <- (rise + approx_end - jd) * 24 + tz
  end_time <- to_dms(ends)

  # Check for leap (skipped) tithi
  moon_phase_tom <- lunar_phase(rise + 1)
  tomorrow <- as.integer(ceiling(moon_phase_tom / 12))
  leap_tithi <- NA_integer_
  leap_end_time <- rep(NA_integer_, 3)
  if (((tomorrow - today) %% 30) > 1) {
    leap_tithi <- today + 1
    degrees_left_leap <- leap_tithi * 12 - moon_phase
    approx_end_leap <- inverse_lagrange(offsets, relative_motion, degrees_left_leap)
